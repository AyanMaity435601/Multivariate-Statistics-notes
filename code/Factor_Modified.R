...
# Factor Analysis
# =======================================================
Stock=read.csv("/home/sysadm/Sourish/Multivariate Statistics/Stock Price Data.csv", header=F)
...
####################################################################################################
StdStockMatFactLoad<-StdStockMatPCA$rotation*t(matrix(rep(StdStockMatPCA$sdev,p),p,p))#[\sqrt(\lambda_1)e_1:...:\sqrt(\lambda_p)e_p]print(StdStockMatFactLoad)  #Loadings on factors,i.e., full L matrix
#---------------------------------------------------------------------------------------------------
eign<-eigen(cor(StockMat))
eign
v<-eign$vectors
p<-eign$values
ani<-sqrt(p[2])%*%v[,2]
...
#==========================================
#mycode:
cumsum(StdStockMatFactLoad[1,]^2)   #for 1 row
cor(StockMat)[1][1]-cumsum(StdStockMatFactLoad[1,]^2)   #for 1 row
######################################################################################################
